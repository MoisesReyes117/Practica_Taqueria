...
public class PromocionesDAO {
    ...
    public void updpromociones(){
        String query = "update promociones set nombre='"+nompromociones+"'" +
                ",descripción='"+descripcion+"',pocentaje="+porcentaje+" where CvePromo="+Cvepromociones;
        try{
            Statement stmt = con.createStatement();
            stmt.executeUpdate(query);
    ...
    public ObservableList<PromocionesDAO> seeAllPromociones(){
        ...
        try{
            ...
            while(res.next()){
                ...
                objP.setPorcentaje(res.getInt("pocentaje"));
                listaP.add(objP);
            }
